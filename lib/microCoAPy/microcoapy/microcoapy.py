...
class Coap:
    ...
    def start(self, port=macros._COAP_DEFAULT_PORT, id=None, psk=None):
        self.sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
        self.sock.bind(('', port))
        self.sock = ssl.wrap_socket(self.sock, identity=id, pre_shared_key=psk)
    ...
    def send(self, ip, port, url, type, method, token, payload, content_format, query_option, observe_option=None):
        packet = CoapPacket()
        packet.type = type
    ...
    # Confirmable
    def ping(self, ip, port, token=bytearray()):
        return self.send(ip, port, "", macros.COAP_TYPE.COAP_CON, macros.COAP_METHOD.COAP_EMPTY_MESSAGE, token, None, macros.COAP_CONTENT_FORMAT.COAP_NONE, None, None)

    def get(self, ip, port, url, token=bytearray(), observe_option=None):
        return self.send(ip, port, url, macros.COAP_TYPE.COAP_CON, macros.COAP_METHOD.COAP_GET, token, None, macros.COAP_CONTENT_FORMAT.COAP_NONE, None, observe_option)
    ...
    def loop(self, blocking=True):
        ...
        while (buffer is not None) and (len(buffer) > 0):
            ...
            self.log("Incoming Packet bytes: " +
                     str(binascii.hexlify(bytearray(buffer))))
            ...
            if self.discardRetransmissions:
                if packet.toString() == self.lastPacketStr:
                    self.log("Discarded retransmission message: " +
                             packet.toString())
                    return False
                else:
            ...
            if self.isServer:
                ...
                # or the data of a separate message
                    ...
                    if self.state == self.TRANSMISSION_STATE.STATE_SEPARATE_ACK_RECEIVED_WAITING_DATA or packet.type == macros.COAP_TYPE.COAP_CON:
                        self.state = self.TRANSMISSION_STATE.STATE_IDLE
